...
const LiveInstructions: React.VFC<LiveInstructionsProps> = ({ steps, isActive, closeHandler }) => {
    const [activeStepIndex, setActiveStepIndex] = useState<number>(0);
    const [activeTimer, setActiveTimer] = useState<boolean>(false);
    const [countdown, setCountdown] = useState<number>(steps[0].duration ? steps[0].duration * 60 : 0);
    ...
    const getPreviewStep = (index: number) => {
        return activeStepIndex + 1 === steps.length ? '' : `Volgende stap: ${getStep(index + 1)}`
    }
            ...
            <Box sx={{
            ...
            }}>
                {
                    !isMobile && <Box sx={{
                        ...
                        <Timeline position="alternate">
                            {
                                steps.map((step, i: number) => <TimelineItem key={i}>
                                    <TimelineSeparator>
                                        <TimelineDot color={i === activeStepIndex ? 'primary' : 'secondary'} />
                                        <TimelineConnector />
                                    </TimelineSeparator>
                ...
                <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        >
                            {activeStepIndex + 1 === steps.length ? 'Sluiten' : 'Volgende Stap'}
                        </Box>
